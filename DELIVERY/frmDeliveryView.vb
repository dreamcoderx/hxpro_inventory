...
" FROM [tbl_delivery_hdr]"
            ...
            dgvRentalHDR.Columns(0).HeaderText = "DR NUMBER"
            dgvRentalHDR.Columns(1).HeaderText = "DELIVERY TO"
            dgvRentalHDR.Columns(2).HeaderText = "ADDRESS"
            dgvRentalHDR.Columns(3).HeaderText = "DATE"
            dgvRentalHDR.Columns(4).HeaderText = "PREPARED BY"
    ...
    Private Sub Button2_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Button2.Click
        Try
            If dgvRentalHDR.CurrentRow.Index > -1 Then
                ...
                a.txtDR_No.Text = (dgvRentalHDR.Item("dr_no", dgvRentalHDR.CurrentRow.Index).Value())
                a.Show()
                Me.Dispose()
            End If
        Catch ex As Exception
    ...
    Private Sub Button1_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Button1.Click
        Try
            If dgvRentalHDR.CurrentRow.Index > -1 Then
                ...
                Dim strRentalID As String = (dgvRentalHDR.Item("dr_no", dgvRentalHDR.CurrentRow.Index).Value())
                a = MessageBox.Show(String.Format("Are you sure you want to delete dr: {0}?", dgvRentalHDR.Item("dr_no", dgvRentalHDR.CurrentRow.Index).Value()), _
                ...
                If a = DialogResult.Yes Then
                    If deleteRental(strRentalID) Then
                        MsgBox("Delivery successfully remove!")
                        loadRentalHDR()
                    Else
                        MsgBox("Delivery failed to remove!")
                    End If
                End If
            End If
        Catch ex As Exception
    ...
    Private Function deleteRental(ByVal strRentalID As String) As Boolean
        ...
        Try
            ...
            con.Open()
            sql = _
"DELETE FROM [tbl_delivery_hdr]" & _
" WHERE [dr_no] = @dr_no"
            cmd = New SqlCommand(sql, con)
            cmd.Parameters.AddWithValue("@dr_no", strRentalID)
            If cmd.ExecuteNonQuery() > 0 Then
                Return True
            End If
            Return False
