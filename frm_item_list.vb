...
Public Class frm_item_list
    ...
    Private Sub LoadGrid()
        ...
        Try
            ...
            sql.Append("select a.item_code, a.asset_number, a.serial_number,")
            sql.Append(" b.part_id, b.description, b.model_no, c.brand_name, e.cat_desc,")
            sql.Append(" d.sub_cat_desc, ")
            sql.Append(" rental.dt_in, rental.dt_out")
            sql.Append(" from tbl_item a")
            sql.Append(" left join tbl_part b On a.part_id  = b.part_id ")
            sql.Append(" left join tbl_brand c On b.brand_id = c.brand_id")
            sql.Append(" left join tbl_sub_category d On b.sub_cat_id = d.sub_cat_id")
            sql.Append(" left join tbl_category e On d.cat_id = e.cat_id")
            sql.Append(" left join (SELECT * FROM ( SELECT *, ")
            sql.Append(" ROW_NUMBER() OVER (PARTITION BY item_code ORDER BY dt_out DESC) As RowNum")
            sql.Append(" from tbl_rental_asset) AS item_rental")
            sql.Append(" WHERE RowNum = 1) rental On a.item_code = rental.item_code ")
            ...
            If cbDeleted.Checked Then
                str_where.Add("a.is_deleted = 1")
            Else
                str_where.Add("(a.is_deleted Is null Or a.is_deleted = 0)")
            End If
            ...
            If str_where_con <> "" Then
                sql.Append(" WHERE " & str_where_con)
            End If

            sql.Append(" order by a.asset_number")

            Dim command As New System.Data.SqlClient.SqlCommand(sql.ToString, connection)
            ...
            dgv_parts.Columns(10).HeaderText = "DATE IN"
            dgv_parts.Columns(11).HeaderText = "DATE OUT"
    ...
    Private Sub LoadGrid2()
        ...
        Try
            ...
            Dim sql As String
            sql =
"select a.item_code, a.asset_number, a.serial_number, b.part_id, b.description, b.model_no, c.brand_name, e.cat_desc, d.sub_cat_desc  from tbl_item a" &
" left join tbl_part b on a.part_id  = b.part_id " &
" left join tbl_brand c on b.brand_id = c.brand_id" &
" left join tbl_sub_category d on b.sub_cat_id = d.sub_cat_id" &
...
"select b.part_id, b.description from tbl_part b"
    ...
    Private Sub list_btw()
        Dim myfolder As String = Path & "/bt/"
        For Each myfile As String In My.Computer.FileSystem.GetFiles _
        (myfolder, FileIO.SearchOption.SearchTopLevelOnly, "*.btw")
            cbo_print.Items.Add(IO.Path.GetFileName(myfile))
        Next
    End Sub
    ...
    Private Sub btn_save_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btn_save.Click
        Try
            ...
            For ctr As Integer = 0 To dgv_parts.RowCount - 1
                ...
                If bolchk Then
                    TW.WriteLine(dgv_parts.Item("asset_number", ctr).Value & "|" & dgv_parts.Item("serial_number", ctr).Value.ToString &
                                 "|" & dgv_parts.Item("description", ctr).Value & "|" & dgv_parts.Item("model_no", ctr).Value &
                                 "|" & dgv_parts.Item("brand_name", ctr).Value)
                End If
            Next
...
" WHERE [asset_number] = @asset_number"
    ...
    Private Sub Button3_Click(sender As Object, e As EventArgs) Handles Button3.Click
        ...
        Dim downloadFolder As New System.Text.StringBuilder(128)
        SHGetKnownFolderPath(FolderDownloads, 0, IntPtr.Zero, downloadFolder)
        ...
        saveLocation.InitialDirectory = downloadFolder.ToString
        saveLocation.Filter = "csv File(.csv)|*.csv"
        If saveLocation.ShowDialog() = DialogResult.OK Then
            File.WriteAllText(saveLocation.FileName, csvStr.ToString)
            MessageBox.Show(saveLocation.FileName + " CSV File Exported Successful...")
        End If
