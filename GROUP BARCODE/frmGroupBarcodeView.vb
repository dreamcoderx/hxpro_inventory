...
Imports System.Xml
Public Class frmGroupBarcodeView
 ...
 " FROM [tbl_group_hdr]"
            ...
            dgvSetList.Columns.Clear()
            dgvSetList.DataSource = Nothing
            dgvSetList.Columns.Add(checkCol)
            dgvSetList.Columns(0).Width = 50

            dgvSetList.DataSource = dtRentalHdr
            dgvSetList.Columns(1).Width = 200
            dgvSetList.Columns(2).Width = 200
            dgvSetList.Columns(3).Width = 700

            dgvSetList.Columns(0).ReadOnly = False
            dgvSetList.Columns(1).ReadOnly = True
            dgvSetList.Columns(2).ReadOnly = True
            dgvSetList.Columns(3).ReadOnly = True

            dgvSetList.Columns(0).HeaderText = "SELECT"
            dgvSetList.Columns(1).HeaderText = "GROUP ID"
            dgvSetList.Columns(2).HeaderText = "BARCODE"
            dgvSetList.Columns(3).HeaderText = "DESCRIPTION"

            dgvSetList.AutoSizeColumnsMode = DataGridViewAutoSizeColumnsMode.AllCells
    ...
    Private Sub Button2_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Button2.Click
        If dgvSetList.CurrentRow.Index > -1 Then
            Dim a As New frmGroupScanAdd
            a.txtGroupID.Text = (dgvSetList.Item(1, dgvSetList.CurrentRow.Index).Value())
            a.ShowDialog()
        End If
    End Sub

    Private Function deleteGroup(ByVal strGroupID As String) As Boolean
        Dim con As New SqlConnection
        Dim cmd As New SqlCommand
        Dim sql As String
        con.ConnectionString = ConnectServer()
        Try
            ...
            If con.State = ConnectionState.Closed Then
                con.Open()
            End If
            sql = "DELETE FROM [tbl_group_hdr]" & _
"WHERE group_id = @group_id"
    ...
    Private Sub Button1_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Button1.Click
        Try
            If dgvSetList.CurrentRow.Index > -1 Then
                ...
                Dim GroupID As String = (dgvSetList.Item(1, dgvSetList.CurrentRow.Index).Value())
                a = MessageBox.Show(String.Format("Are you sure you want to delete this group:{0}?", dgvSetList.Item(1, dgvSetList.CurrentRow.Index).Value()), _
                                             "Warning!", MessageBoxButtons.YesNo, MessageBoxIcon.Exclamation, MessageBoxDefaultButton.Button1)
                If a = DialogResult.Yes Then
                    If deleteGroup(GroupID) Then
                        MsgBox("Group Barcode successfully deleted!")
                        loadSetBarcode()
                    Else
                        MsgBox("Group Barcode failed to delete!")
                    End If
                End If
            End If
        Catch ex As Exception
    ...
    Private Sub Button4_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Button4.Click
        Dim a As New frmGroupCreateHdr
        a.ShowDialog()
    End Sub
    ...
    Private Sub btn_save_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btn_save.Click
        Try
            ...
            For ctr As Integer = 0 To dgvSetList.RowCount - 1
                Dim bolchk As New Boolean
                bolchk = dgvSetList.Item(0, ctr).Value
                If bolchk Then
                    TW.WriteLine(dgvSetList.Item(2, ctr).Value & "|" & dgvSetList.Item(3, ctr).Value.ToString)
                End If
            Next
