...
" where a.asset_number = '" & txtAssetNumber.Text & "'"
    ...
    Private Function InsertReturn(ByVal strItemCode As String) As Boolean
        ...
        Try
            ...
            sql = _
"INSERT INTO [tbl_repair_return]" & _
"([item_code]" & _
", [status]" & _
...
", @status" & _
", @repair_description" & _
", @dr_number, @date);SELECT SCOPE_IDENTITY()"
            cmd = New SqlCommand(sql, con)
            ...
            cmd.Parameters.AddWithValue("@dr_number", txtDR.Text)
...
" SET [repair_return_id] = @repair_return_id" & _
" WHERE [item_code] = @item_code and [repair_return_id] is null"
                cmd = New SqlCommand(sql, con)
                cmd.Parameters.AddWithValue("@repair_return_id", insertID)
                cmd.Parameters.AddWithValue("@item_code", strItemCode)
                cmd.ExecuteNonQuery()
                Return True
            Else
...
" WHERE a.repair_id = '" & "xxxx" & "'"
    ...
    Private Sub Button1_Click_1(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Button1.Click
        If strItemCode <> "" Or txtDescription.Text <> "" Or txtDR.Text <> "" Then
        ...
        End If
    End Sub
    Private Function loadRepairDR() As Boolean
        Dim con As New SqlConnection
        Dim cmd As New SqlCommand
        Dim sql As String
        con.ConnectionString = ConnectServer()
        Try
            ...
            End If
            sql = _
"select b.asset_number" & _
" , c.description" & _
", c.model_no" & _
", d.brand_name" & _
", a.status" & _
", a.repair_description" & _
" from tbl_repair_return a" & _
" left join tbl_item b on a.item_code = b.item_code" & _
" left join tbl_part c on b.part_id = c.part_id" & _
" left join tbl_brand d on d.brand_id = c.brand_id" & _
" where a.dr_number = '" & txtDR.Text & "'"
            cmd = New SqlCommand(sql, con)
    ...
    Private Sub txtDR_KeyPress(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyPressEventArgs) Handles txtDR.KeyPress
        Dim Keyascii As Integer = AscW(e.KeyChar)
        If Keyascii = 13 And txtDR.Text <> "" Then
            loadRepairDR()
        End If
    End Sub
